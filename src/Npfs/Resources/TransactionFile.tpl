...
: Resource(filename, parent),
  beginTxnHandler(0), beginTxnRef(~uintptr_t(0)),
  abortTxnHandler(0), abortTxnRef(~uintptr_t(0)),
  commitTxnHandler(0), commitTxnRef(~uintptr_t(0)),
  endTxnHandler(0), endTxnRef(~uintptr_t(0))
{
  // deny all accesses
  stat.mode &= ~0777;
...
template<class SerDes>
PT_THREAD(TransactionFile<SerDes>::openCoRoutine(ResultMessage& result, Npfs::OpenIoState& workRef))
{
  ...
  bool writeAccess = ioRef->flag.wasOpenedForWrite = (((mode & 0x03) == Owrite) || ((mode & 0x03) == Ordwr));

  if (beginTxnHandler)
    PT_WAIT_UNTIL(pt, beginTxnHandler->execute(ioRef->transientValue, writeAccess, txnError, beginTxnRef));
  ...
  // check, if the beginTxnHandler denied write access
  if (ioRef->flag.wasOpenedForWrite and not writeAccess)
  {
...
void TransactionFile<SerDes>::flush(Npfs::OpenIoState& workRef)
{
  ...
  {
    IoBuffer* ioRef = static_cast<IoBuffer*>(workRef.ioState);
    abortTxnHandler->execute(ioRef->transientValue, ioRef->flag.wasOpenedForWrite, abortTxnRef);
  }
